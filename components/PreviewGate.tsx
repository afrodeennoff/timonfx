...
const TerminalLine: React.FC<{ label: string; value: string; delay?: number }> = ({ label, value, delay = 0 }) => (
  ...
  >
    ...
    <span className="mono text-[9px] text-brand-purple font-black tracking-widest group-hover/line:text-white transition-colors duration-300">{value}</span>
  </motion.div>
);
    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          >
             <div className={`relative overflow-hidden ${GLASS_STYLES.card} p-10 md:p-14 group/main`}>
                   ...
                   <div className="w-full md:w-1/2 space-y-6">
                      <div className="space-y-4">
                        <div className="flex items-center gap-3">
                           ...
                           <span className="mono text-[10px] text-brand-purple font-black tracking-[0.5em] uppercase italic">Gate_Online</span>
                        </div>
                   ...
                   <div className="w-full md:w-1/2 space-y-6">
                      <div className="space-y-4">
                        <div className="space-y-2">
                           <span className="mono text-[9px] text-zinc-600 font-black tracking-widest uppercase">Clearance Required</span>
                           <motion.input 
                             placeholder="AUTH_CODE_KEY..." 
                             value={authCode}
                             onChange={(e) => setAuthCode(e.target.value)}
                             whileFocus={VARIANTS.inputFocus}
                             className="w-full bg-zinc-950 border border-white/15 p-4 mono text-[11px] text-white uppercase tracking-[0.3em] focus:outline-none focus:border-brand-purple font-black placeholder:text-zinc-800 transition-colors duration-300 rounded-[1.25rem]"
                           />
                        </div>
                        ...
                        <motion.button 
                           ...
                           className={`group relative w-full py-4 mono text-[11px] font-black text-white uppercase tracking-[0.4em] ${isVerifying ? 'opacity-50 cursor-wait' : `${GLASS_STYLES.button} ${GLASS_STYLES.buttonHover}`}`}
                        >
                      ...
                      <motion.button 
                        ...
                        whileTap={VARIANTS.buttonTap}
                        onClick={onClose}
                        className={`w-full py-3.5 mono text-[9px] text-zinc-600 hover:text-white uppercase tracking-[0.6em] font-black ${GLASS_STYLES.button} ${GLASS_STYLES.buttonHover}`}
                      >
                        Abort
                      </motion.button>
                   </div>
